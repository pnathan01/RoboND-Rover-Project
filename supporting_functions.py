...
def create_output_images(Rover):
      ...
      if rock_world_pos[0].any():
            ...
            for idx in range(len(Rover.samples_pos[0])):
                  ...
                  accurate_dist = rock_sample_dists
                  Rover.sample_distance = np.min(rock_sample_dists)
                  # If rocks were detected within 3 meters of known sample positions
                  # consider it a success and plot the location of the known
                  # sample on the map
                # my code begins
                  if Rover.sample_distance < 0.075:
                  #if np.min(rock_sample_dists) < 3 and np.min(rock_sample_dists) > 1:
                        # my code ends
                        map_add[test_rock_y-rock_size:test_rock_y+rock_size, 
                        test_rock_x-rock_size:test_rock_x+rock_size, :] = 255
                        # My code begins
                        print('Near sample supporting function')
                        print(Rover.pos)
                        print(Rover.samples_pos[0][idx])
                        print(Rover.samples_pos[1][idx])
                        Rover.throttle = 0
                        Rover.steer = 0
                        ...
                        print('Sample turn angle mid range:')
                        print(turn_angle)
                        print('Sample distance mid range:')
                        print(Rover.sample_distance)
                        print('Acc dist:')
                        print(accurate_dist)
                  ...
                  #elif np.min(rock_sample_dists) < 0.05:
                    #print('Sample turn angle near range:')
                    #print(turn_angle)
                    #print('Sample distance near range:')
